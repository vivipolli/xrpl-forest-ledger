...
class UsbApi extends abstract_1.AbstractApi {
    ...
    async read(path, signal) {
        ...
        try {
            ...
            const res = await this.abortableMethod(() => device.transferIn(this.debugLink ? constants_1.DEBUGLINK_ENDPOINT_ID : constants_1.ENDPOINT_ID, this.chunkSize), { signal, onAbort: () => device?.reset() });
    ...
    async write(path, buffer, signal) {
        ...
        try {
            ...
            const result = await this.abortableMethod(() => device.transferOut(this.debugLink ? constants_1.DEBUGLINK_ENDPOINT_ID : constants_1.ENDPOINT_ID, newArray), { signal, onAbort: () => device?.reset() });
    ...
    async openDevice(path, first, signal) {
        for (let i = 0; i < 5; i++) {
    ...
    async openInternal(path, first, signal) {
        const device = this.findDevice(path);
        if (!device) {
            return this.error({ error: ERRORS.DEVICE_NOT_FOUND });
        }
